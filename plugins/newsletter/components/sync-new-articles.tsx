...
`
  const articleReferenceBlock = content?.find((block: any) => block._type === TYPE_REFERENCES) ?? {
    '_key': nanoid(10),
    '_type': 'articleReferences',
    'references': []
  }
  ...
  const handleFetch = useCallback(async () => {
    ...
    const existingReferenceIds =
      [
        ...new Set(
          queryResult?.map((newsletter: SanityDocumentLike) => {
            const content = (newsletter?.content || []) as ArticleReferencesBlock[]
            return (content?.filter((block) => block._type === TYPE_REFERENCES)?.map((block: ArticleReferencesBlock) => block.references?.map((reference) => reference._ref) ?? []) ?? []).flat().filter(Boolean)
          }).flat() ?? []
        )
      ]
  ...
  const hasPendingArticles = useMemo(() => pendingArticleCount > 0, [pendingArticleCount])

  const existingReferenceIds = (content?.filter((block) => block._type === TYPE_REFERENCES)?.map((block: ArticleReferencesBlock) => block.references?.map((reference) => reference._ref) ?? []) ?? []).flat().filter(Boolean)
    ...
    <Stack space={4}>
      ...
      <Card>
        <Flex gap={2} align="center">
          <Button tone="primary" loading={isLoading} disabled={isLoading} text="Pull in new articles"
                  onClick={handleSync} />
          {pendingArticleCount ? (
            <Box padding={2}>
              <Text size={2} muted>
                {pendingArticleCount} new {pendingArticleCount === 1 ? 'article' : 'articles'} to sync
              </Text>
            </Box>
          ) : null}
        </Flex>
